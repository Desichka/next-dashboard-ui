...
interface EventModalProps {
  ...
  onSave: (event: Omit<FullCalendarEventForModal, 'id' | 'extendedProps'> & { id?: string; extendedProps?: Partial<FullCalendarEventForModal['extendedProps']> }) => void;
...
const EventModal: React.FC<EventModalProps> = ({ isOpen, onClose, onSave, onDelete, slotInfo, eventToEdit }) => {
  ...
  const handleSave = () => {
    if (!title || !startDate) { // End date might be optional depending on logic
    ...
    const eventData: Omit<FullCalendarEventForModal, 'id' | 'extendedProps'> & { id?: string; extendedProps?: Partial<FullCalendarEventForModal['extendedProps']> } = {
  ...
  const formatDateForInput = (date: Date | null): string => {
    ...
    } catch (e) { return ''; } // Handle potential invalid date
  };
  ...
  const handleDateChange = (e: React.ChangeEvent<HTMLInputElement>, type: 'start' | 'end') => {
      ...
      if (!dateValue) { // Handle empty input
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50 p-4">
      <div className="bg-white dark:bg-gray-800 p-6 rounded shadow-lg w-full max-w-md max-h-[90vh] overflow-y-auto modal-content-scroll"> {/* Added class */}
        ...
        {/* Color Picker */}
        <div className="mb-4">
            <label htmlFor="eventColor" className="block text-sm font-medium mb-1">Color</label>
            <input
                type="color"
                id="eventColor"
                value={color}
