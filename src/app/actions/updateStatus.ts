...
export async function updateItemStatus(input: UpdateStatusInput): Promise<{ success: boolean; message: string; data?: any }> {
  ...
  if (isNaN(itemId)) { // Check if parsing resulted in NaN
  ...
  try {
    const updatedItem = await prisma.design.update({ // Use the parsed numeric ID
      where: { id: itemId },
    ...
    if (error.code === 'P2025') { // Use the original string ID for the error message if needed, or the parsed ID
